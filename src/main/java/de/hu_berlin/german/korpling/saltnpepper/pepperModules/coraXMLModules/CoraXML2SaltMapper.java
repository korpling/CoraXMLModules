...
public class CoraXML2SaltMapper extends PepperMapperImpl implements PepperMapper, CoraXMLDictionary
{
   ...
   class CoraXMLReader extends DefaultHandler2
   {
    ...
    private String unescape(Attributes attributes) {
        //return attributes.getValue(ATT_TAG);
    ...
    private void addSimpleRow(String name, Attributes attributes, String ns) {
        getSNodeStack().peek().createSAnnotation(ns, name, unescape(attributes));
    }
      ...
      public void startElement(   String uri,
                              ...
                              Attributes attributes)throws SAXException
       {
         if (TAG_LAYOUTINFO.equals(qName)){
            ...
            span.createSAnnotation(null, SEGMENTATION_NAME_MOD, StringEscapeUtils.unescapeHtml4(attributes.getValue(ATT_SIMPLE)));
            ...
            // XXX TODO(fpetran):
            // it seems pages start at 0 in CoraXML when converted from BonnXML
            // check if that is really always the case, and think hard about whether to fix it in
    ...
    else if (TAG_POS_INT_LEMMA.equals(qName)) {
        addSimpleRow("posLemma_intern", attributes);
    } else if (TAG_POS_INT.equals(qName)) {
        addSimpleRow("pos_intern", attributes);
    } else if (TAG_INFL_INT.equals(qName)) {
